...
export async function GET(request: NextRequest) {
  try {
    const rateLimitResponse = enforceRateLimit(request, 'admin-challenge', {
      max: 5,
      windowMs: 60_000,
    });
    ...
    if (rateLimitResponse) {
      await logSecurityEvent({
        action: 'admin_challenge_rate_limited',
    ...
    if (!walletAddress) {
      await logSecurityEvent({
        action: 'admin_challenge_missing_wallet',
        metadata: { ipHash: getRequestIpHash(request) },
        outcome: 'failure',
        request,
        route: '/api/admin/challenge',
    ...
    const challenge = await createSignedChallenge(walletAddress, 'admin');
    const cookieStore = await cookies();

    cookieStore.set(ADMIN_CHALLENGE_COOKIE, challenge.challengeToken, {
      httpOnly: true,
